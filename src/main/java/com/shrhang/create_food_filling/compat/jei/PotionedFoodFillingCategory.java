...
public class PotionedFoodFillingCategory implements IRecipeCategory<PotionedFoodFillingRecipe> {
    ...
    public PotionedFoodFillingCategory(IDrawable icon) {
        this.icon = icon;
        this.slot = asDrawable(AllGuiTextures.JEI_SLOT);
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, PotionedFoodFillingRecipe recipe, IFocusGroup focuses) {
        ...
        builder
                ...
                .addIngredients(ForgeTypes.FLUID_STACK, recipe.fluids())
    ...
    @Override
    public void draw(PotionedFoodFillingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
        AllGuiTextures.JEI_SHADOW.render(graphics, 62, 57);
        AllGuiTextures.JEI_DOWN_ARROW.render(graphics, 126, 29);

        FluidStack displayedFluid = recipeSlotsView.findSlotByName(FLUID_SLOT)
                .flatMap(slot -> slot.getDisplayedIngredient(ForgeTypes.FLUID_STACK))
                .orElseGet(() -> recipe.fluids().get(0));
    ...
    private static void addPotionFluidTooltip(IRecipeSlotView view, ITooltipBuilder tooltip) {
        ...
        PotionUtils.addPotionTooltip(PotionUtils.getAllEffects(displayed.get().getTag()), potionTooltip, 1.0F);
